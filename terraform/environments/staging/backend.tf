...
# under its own key path (staging/comprobify-web/... - the API repo uses
# staging/comprobify/...) - the S3 backend's `key` is what scopes state, so this is fully
# independent despite sharing a bucket. Use a Spaces access key dedicated to this repo's
# pipeline, not the API repo's - that's the real credential-isolation boundary, not the
# bucket choice.
#
