...
export async function setupIssuerAction(formData: FormData): Promise<SettingsResult> {
  try {
    ...
    const fields: IssuerRegistrationFields = {
      ...
      branchCode: ((formData.get('branchCode') as string | null) || '001').trim(),
      issuePointCode: ((formData.get('issuePointCode') as string | null) || '001').trim(),
      emissionType: '1',
    ...
    try {
      ...
      console.error('Unexpected error calling registerIssuer:', err);
