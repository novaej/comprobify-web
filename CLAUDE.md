...
| Auth | Auth.js v5 (next-auth@beta) — per-user API key stored in PostgreSQL via Prisma |
...
src/
  ...
  components/
    ...
    sandbox-banner.tsx      Yellow banner when session environment === 'sandbox'
...
1. **Never import `src/lib/api.ts` in client components** — it is `server-only` and calls `requireApiKey()` which reads the user's API key from the database. Importing it in a client component will throw at build time.
2. **Never expose the user's Comprobify API key to the browser** — it lives only in the `users` table (never in the JWT or session). Do not pass it through props or return it from Server Actions.
...
**Sandbox mode:** The `environment` column on the `users` table (`'sandbox'` | `'production'`) drives the yellow `SandboxBanner` and the environment badge in Settings. It is written during issuer setup and flipped to `'production'` on promotion. There is no `COMPROBIFY_SANDBOX` env var — sandbox state is per-user, not global.
...
9. Leaking the user's Comprobify API key to the browser — it must stay server-side only, fetched via `requireApiKey()` from the DB. Never pass it as a prop, include it in the session, or return it from a Server Action.
...
| `src/proxy.ts` | next-intl routing middleware (locale detection + redirect) |
