...
| `src/app/actions/credit-note.ts` | `createCreditNoteAction`/`rebuildCreditNoteAction` (same shape as invoice.ts's pair); `getInvoiceForCreditNoteAction(accessKey)` resolves an AUTHORIZED invoice into `originalDocument`/`buyer`/`items` prefill data (reconstructing the `NNN-NNN-NNNNNNNNN` number from the issuer's `branchCode`/`issuePointCode` + the document's `sequential`); `searchCreditableInvoicesAction(sequential)` wraps `listDocuments` for the "Buscar comprobante" picker; exports `CreditNoteFormData` type |
