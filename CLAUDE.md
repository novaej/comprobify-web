...
**Issuer editing (logo + certificate, not business data):** The Comprobify API has no update endpoint for an existing issuer's business data (RUC, name, address) — that's set once at creation (`POST /v1/issuers`) and never changed afterward. Two fields *are* mutable post-creation: the logo, via `PATCH /v1/issuers/:id/logo` (multipart, PNG/JPEG/GIF, ≤500KB, used in PDF/RIDE generation, never displayed in the web UI), and the P12 signing certificate, via `PATCH /v1/issuers/:id/certificate` (multipart, `cert` file + optional `certPassword`, e.g. when the cert expires). `/issuers` exposes both as per-card dialogs (Owner/Admin, `issuers.manage`): "Cambiar logo" calling `updateIssuerLogoAction` → `uploadIssuerLogo()`, and "Renovar certificado" calling `renewIssuerCertificateAction` → `renewIssuerCertificate()`, both in `src/lib/api.ts`. Renewing the certificate only changes what's used for *future* signing — each already-signed document embeds its own certificate copy inside the XML signature, so past invoices are unaffected. The same page also renders cert expiry + fingerprint per issuer by calling `listTenantIssuers()` (a tenant-wide `GET /v1/issuers` call) and matching each row to the local `Issuer` by `String(issuer.apiIssuerId) === apiIssuer.id` — the local Prisma `Issuer` table does not store cert fields, only the API does; both mutation actions call `revalidatePath('/issuers')` so the updated fingerprint/expiry show up without a manual reload. The expiry badge thresholds (>30d ok, 8–30d warning, ≤7d/expired critical) mirror `comprobify/src/services/notification.service.js`'s `CERT_WARN_DAYS`/`CERT_ERROR_DAYS` so the UI agrees with the cert-expiry notifications.
...
| `src/app/actions/issuers.ts` | `createBranchAction`, `addDocumentTypeAction`, `removeDocumentTypeAction`, `updateIssuerLogoAction`, `renewIssuerCertificateAction` |
| `src/components/issuer-manager.tsx` | Client Component — issuer cards on `/issuers`: document type add/remove, cert expiry/fingerprint badge, "Cambiar logo" dialog, "Renovar certificado" dialog |
