...
| `src/app/[locale]/invoices/[key]/page.tsx` | Invoice Detail Server Component — fetches document + events, renders `InvoiceActions` (passing its own validated `from`) and the events timeline |
| `src/components/invoice-actions.tsx` | Client Component — status-driven action buttons: "Enviar" (`SIGNED`), "Corregir" linking to `/invoices/new?rebuild=...` (`RETURNED`/`NOT_AUTHORIZED`), download/resend (`AUTHORIZED`) |
