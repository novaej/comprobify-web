...
**PDF preview on Invoice Detail:** For `AUTHORIZED` documents, `InvoicePdfPreviewToggle` (`src/components/invoice-pdf-preview-toggle.tsx`) renders a single "Vista previa PDF" button — only the icon (`Eye`/`EyeOff`) changes with state, not the label — and lazy-mounts `InvoicePdfPreview` only once clicked. That component is wrapped in `invoice-pdf-preview-lazy.tsx`, a tiny Client Component whose sole job is calling `next/dynamic(..., { ssr: false })`: Next.js's Turbopack build rejects `ssr: false` called directly from a Server Component (a hard build error, not just a warning), so the dynamic-import call must live in its own Client Component file, never inline in a page. `InvoicePdfPreview` itself (`react-pdf` + `pdfjs-dist`) points `<Document file={...}>` at the existing `/api/documents/:key/ride` download route — no separate endpoint needed — and uses a `ResizeObserver` on its container to keep the rendered page responsive (rule 11). See Common Mistake #27 for the worker-loading gotcha specific to this library.
...
| `src/components/invoice-actions.tsx` | Client Component — status-driven action buttons: "Enviar" (`SIGNED`), "Corregir" linking to `/invoices/new?rebuild=...` or `/credit-notes/new?rebuild=...` per `REBUILD_HREFS[documentType]` (`RETURNED`/`NOT_AUTHORIZED`), "Crear nota de crédito" (`AUTHORIZED` type `01` only), download/resend (`AUTHORIZED`) |
...
| `src/components/invoice-pdf-preview-toggle.tsx` | "Vista previa PDF" toggle button shown for `AUTHORIZED` documents; lazy-mounts the preview only once clicked |
...
| `src/components/invoice-pdf-preview.tsx` | Renders `/api/documents/:key/ride` via `react-pdf`; `ResizeObserver`-driven responsive width, Previous/Next controls for multi-page PDFs |
