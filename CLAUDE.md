...
13. Wrapping `redirect()` in a broad `try/catch` without re-throwing `NEXT_REDIRECT` — `redirect()` works by throwing a special error with `digest` starting with `'NEXT_REDIRECT'`. If a catch block swallows it (e.g. returning `{ error: 'UNEXPECTED_ERROR' }`), the redirect never fires. Re-throw it: `if ((err as { digest?: string }).digest?.startsWith('NEXT_REDIRECT')) throw err;`
...
| `src/components/invoice-form.tsx` | Invoice creation form (React Hook Form + Zod); accepts `InvoiceCatalogs` prop; catalog-driven selects, Consumidor Final auto-fill, single-payment auto-sync |
| `src/app/[locale]/invoices/new/page.tsx` | Server Component — fetches catalogs in parallel, passes as props to `InvoiceForm`; exports `InvoiceCatalogs` type |
