...
**`NEXT_PUBLIC_*` values and `SENTRY_AUTH_TOKEN` must be passed as Docker build args, not just runtime env vars.** Next.js inlines `NEXT_PUBLIC_*` into the client (and server) bundle during `next build`, and the Sentry webpack/turbopack plugin that uploads source maps only runs during that same build step — neither is read again once the container starts. The Dockerfile declares `ARG`/`ENV` pairs for `NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_MARKETING_URL`, `NEXT_PUBLIC_APP_ENV`, `NEXT_PUBLIC_SENTRY_DSN`, and `SENTRY_AUTH_TOKEN`; `.github/workflows/deploy-staging.yml`'s `docker build` step passes each via `--build-arg`. Miss one of these and the symptom is subtle — the app still boots and mostly works, but client-side Sentry silently has no DSN, or `robots.ts`/`sitemap.ts` (which read `NEXT_PUBLIC_APP_ENV` server-side too) behave as if running in the wrong environment.
