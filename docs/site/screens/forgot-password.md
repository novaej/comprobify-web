...
Public, unauthenticated flow letting a user with a forgotten password regain access on their own, without an Owner/Admin's help. Backed entirely by `src/app/actions/auth.ts` — `requestPasswordResetAction()` and `resetPasswordAction()` — and two new columns on the local `User` table (`passwordResetTokenHash`, `passwordResetTokenExpiresAt`); no Comprobify API call is involved, since login credentials are local to this app.
...
The token is `randomBytes(32).toString('hex')`, SHA-256-hashed before being persisted to `passwordResetTokenHash` (`@unique` on `User`), with `passwordResetTokenExpiresAt` set to 1 hour out. Only the hash is ever stored — the raw token exists only in the emailed link, so a database compromise alone can't produce a usable reset link. The link is `${NEXT_PUBLIC_APP_URL}/${locale}/reset-password?token=${rawToken}`, sent via `src/lib/mailgun.ts`'s `sendMail()` (the same sender `users.ts` already uses for invite/admin-reset emails) using the `email.forgotPassword` i18n namespace — kept separate from the existing `email.passwordReset` namespace, whose copy assumes an administrator initiated the reset (`{businessName}` interpolation).
...
`resetPasswordAction` re-hashes the submitted token and looks up the user by the unique index, rejecting (generic `INVALID_OR_EXPIRED_RESET_TOKEN`, never distinguishing "no such token" from "expired" from "user disabled") if there's no match, the account is inactive, or `passwordResetTokenExpiresAt` has passed — this is the real security check; `checkResetTokenValid()` on page load is purely a rendering decision and changes nothing about what's enforced at submit. On success it hashes the new password, clears both token columns (making the token single-use — a resubmission or a re-opened link now hashes to a value matching nothing), signs the user in, and calls the same `postLoginRedirect()` shared with `loginAction`/`completeRegistrationAction`.
...
| `prisma/schema.prisma` | `User.passwordResetTokenHash` (`@unique`), `User.passwordResetTokenExpiresAt` |
