...
- A billing mutation succeeds against production — e.g. `/settings/billing`'s subscribe or change-tier flow, or a proof upload. This exercises the *same* `INTERNAL_SERVICE_SECRET` check as registration (ADR-035's addendum), but registration succeeding doesn't guarantee billing works too if the two were ever tested at different times against different secret values — verify both independently.
